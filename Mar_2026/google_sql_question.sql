...
combine_both as (
    select 
        c.*,
        s.customer_id as seller_id
    from 
        get_customer_trans c
    inner join get_seller_trans s
    on c.amount = s.amount and c.tran_date = s.tran_date
),
final_cte as (
select 
    customer_id, 
    seller_id,
    sum(amount) as total_sales_each_combination
from combine_both
group by customer_id, seller_id
order by total_sales_each_combination desc)

select * from final_cte
select 
* from final_cte 
where customer_id not in (select seller_id from final_cte)
union 
select 
* from final_cte
where seller_id not in (select customer_id from final_cte)